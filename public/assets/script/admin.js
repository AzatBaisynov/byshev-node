...
const getGoal = async () => {
	const info = await fetch('http://139.28.37.111:8080/v1/api/file/status', {
		method: 'GET'
	})
...
goalBtn.addEventListener('click', async (e) => {
	...
	fetch('http://139.28.37.111:8080/v1/api/project/status', {
		method: 'POST',
		headers: {
...
const sendImagePdf = async (file, id, type) => {
	...
	try {
		await fetch(`http://139.28.37.111:8080/v1/api/project/${type}/${id}`, {
			method: 'POST',
			headers: {
				'Authorization': localStorage.getItem('auth')
			},
...
cardSave.addEventListener('click', async (e) => {
	e.preventDefault();
	fetch('http://139.28.37.111:8080/v1/api/project/create', {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			'Authorization': localStorage.getItem('auth')
...
const getCards = async () => {
	try {
		const json = await fetch('http://139.28.37.111:8080/v1/api/project/', {
		method: 'GET',
		headers: {
			'Authorization': localStorage.getItem('auth')
		},
		...
		const dataStr = `
			<div class="card">
					<div class="card__image">
							<img src="http://139.28.37.111:8080/v1/api/file/${el?.image?.name}">
					</div>
	...
	dels.forEach((el) => {
		el.addEventListener('click', () => {
			fetch(`http://139.28.37.111:8080/v1/api/project/delete/${el.id.substring(1)}`, {
				method: 'POST',
				headers: {
					'Authorization': localStorage.getItem('auth')
				},
